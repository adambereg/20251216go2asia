...
function validateGatewayClaims(payload: Record<string, unknown>): { ok: true } | { ok: false; error: string } {
  const iss = getStringClaim(payload, 'iss');
  if (iss !== 'api-gateway') return { ok: false, error: 'Invalid issuer' };

  const aud = getStringClaim(payload, 'aud');
  if (aud !== 'internal') return { ok: false, error: 'Invalid audience' };
...
async function verifyGatewayToken(
  ...
  const claims = validateGatewayClaims(verified.payload);
  if (!claims.ok) {
...
export async function getOptionalGatewayPrincipal(
  ...
  requestId: string,
  logger: ReturnType<typeof createLogger>
): Promise<{ ok: true; principal: GatewayPrincipal | null } | { ok: false; res: Response }> {
  if (!request.headers.get('X-Gateway-Auth')) {
    return { ok: true, principal: null };
  }

  const verified = await verifyGatewayToken(request, env, requestId, logger);
