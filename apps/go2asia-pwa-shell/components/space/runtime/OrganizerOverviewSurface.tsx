        ...
        <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
          ...
          <aside className="space-y-4 lg:border-l lg:border-slate-100 lg:pl-8">
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              ...
              <p className="mt-3 text-sm leading-relaxed text-slate-600">{focusAction.whyNow}</p>
            </div>
          ...
          return (
            <div key={group.id}>
              <div className="mb-2.5 flex items-center gap-3">
                ...
                <span className="ml-auto text-xs text-slate-400">{group.actions.length}</span>
              </div>
              <div className={`overflow-hidden rounded-2xl border bg-white ${muted ? 'border-slate-100' : 'border-slate-200'} shadow-sm`}>
                {group.actions.map((action) => (
                  <Link
                    ...
                    className="flex w-full items-start gap-3 border-b border-slate-100 px-4 py-3.5 text-left transition last:border-b-0 hover:bg-slate-50"
                  >
