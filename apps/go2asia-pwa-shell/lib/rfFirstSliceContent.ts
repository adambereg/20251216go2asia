...
export const rfOffersCatalogContent = {
  ...
  claimNotReadyTitle: 'Сохранить предложение',
  claimNotReadyBody:
...
export const rfMyVouchersPageContent = {
  ...
  localWarning:
    'Список хранится локально в этом браузере: для авторизованного пользователя он привязан к текущему аккаунту, для гостя — к этому браузеру. Это сохранённые предложения, не серверные ваучеры.',
  empty: 'Пока пусто. Добавьте оффер из каталога предложений кнопкой «Сохранить предложение».',
