...
export const rfMyVouchersPageContent = {
  ...
  localWarning:
    'Этот список хранится только в этом браузере: для авторизованного пользователя он привязан к текущему аккаунту, для гостя — к этому браузеру. Это не серверный ваучер.',
