...
export const rfMyVouchersPageContent = {
  ...
  meaningBody:
    'Ваучеры показываются как фактологический read-only слой: статус, тип (Standard/Points-enabled), повторяемость и источник через PRO при подтверждённой атрибуции. Это не раздел выплат, наград или комиссий.',
