...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const logger = createLogger(requestId, 'organizer-service', {
      env: env.ENVIRONMENT,
      version: env.VERSION,
    });
    ...
    try {
      ...
      let principal = null;
      if (path.startsWith('/v1/organizer/')) {
      ...
      response = (await handleTripsRoute(request, env, requestId, principal)) ?? handleNotFound(path, requestId);
