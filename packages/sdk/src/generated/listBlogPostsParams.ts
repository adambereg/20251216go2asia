/**
 * Generated by orval v7.17.0 🍺
 * Do not edit manually.
 * Go2Asia Platform API
 * Unified API specification for all Go2Asia backend microservices. This file defines the global schema and shared components. Each microservice extends this file via modular OpenAPI fragments.

 * OpenAPI spec version: 0.1.0
 */
import type { LimitParameter } from "./limitParameter";
import type { ListBlogPostsSort } from "./listBlogPostsSort";

export type ListBlogPostsParams = {
  /**
   * @minimum 1
   * @maximum 200
   */
  limit?: LimitParameter;
  /**
   * Opaque pagination cursor (base64url)
   */
  cursor?: string;
  sort?: ListBlogPostsSort;
  /**
   * Full-text-ish search over title/excerpt/content
   */
  q?: string;
  /**
   * Filter by tag slug
   */
  tag?: string;
  /**
   * Filter by author slug
   */
  author?: string;
  /**
   * Exclude one post (for related lists)
   */
  exclude_slug?: string;
};
