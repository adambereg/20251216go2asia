/**
 * Generated by orval v7.17.0 🍺
 * Do not edit manually.
 * Go2Asia Platform API
 * Unified API specification for all Go2Asia backend microservices. This file defines the global schema and shared components. Each microservice extends this file via modular OpenAPI fragments.

 * OpenAPI spec version: 0.1.0
 */
import type { SpaceGroupStatus } from "./spaceGroupStatus";
import type { SpaceGroupVisibility } from "./spaceGroupVisibility";

export interface SpaceGroupResponse {
  createdAt: string;
  /** @nullable */
  description?: string | null;
  id: string;
  membersCount: number;
  ownerId: string;
  slug: string;
  status: SpaceGroupStatus;
  title: string;
  updatedAt: string;
  visibility: SpaceGroupVisibility;
}
