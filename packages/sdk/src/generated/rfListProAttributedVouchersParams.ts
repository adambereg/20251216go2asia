/**
 * Generated by orval v7.17.0 🍺
 * Do not edit manually.
 * Go2Asia Platform API
 * Unified API specification for all Go2Asia backend microservices. This file defines the global schema and shared components. Each microservice extends this file via modular OpenAPI fragments.

 * OpenAPI spec version: 0.1.0
 */
import type { RfListProAttributedVouchersStatus } from "./rfListProAttributedVouchersStatus";
import type { RfListProAttributedVouchersClaimScope } from "./rfListProAttributedVouchersClaimScope";

export type RfListProAttributedVouchersParams = {
  /**
   * @minimum 1
   * @maximum 100
   */
  limit?: number;
  cursor?: string;
  /**
 * Legacy compatibility filter by runtime `status`. Lifecycle reads should prefer response `canonicalStatus`; `claimed` maps to canonical `available`.

 */
  status?: RfListProAttributedVouchersStatus;
  partnerId?: string;
  claimScope?: RfListProAttributedVouchersClaimScope;
};
