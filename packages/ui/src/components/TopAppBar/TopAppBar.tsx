...
export const TopAppBar: React.FC<TopAppBarProps> = ({
  ...
  return (
    <header className={cn('sticky top-0 z-50 bg-white border-b border-slate-200 shadow-sm', className)}>
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              <div className="relative" ref={userMenuRef}>
                <button
                  onClick={() => {
                    // Если меню не предусмотрено (нет signOut), оставляем старое поведение
                    if (!onSignOutClick) {
                      onProfileClick?.();
                      return;
                    }
