/**
 * Generated by orval v7.17.0 🍺
 * Do not edit manually.
 * Go2Asia Platform API
 * Unified API specification for all Go2Asia backend microservices. This file defines the global schema and shared components. Each microservice extends this file via modular OpenAPI fragments.

 * OpenAPI spec version: 0.1.0
 */

export type AttachMediaUsageRequestOwnerType =
  (typeof AttachMediaUsageRequestOwnerType)[keyof typeof AttachMediaUsageRequestOwnerType];

// eslint-disable-next-line @typescript-eslint/no-redeclare
export const AttachMediaUsageRequestOwnerType = {
  user: "user",
  space_post: "space_post",
  rielt_listing: "rielt_listing",
  rf_partner: "rf_partner",
  quest_submission: "quest_submission",
  blog_post: "blog_post",
  atlas_entity: "atlas_entity",
} as const;
