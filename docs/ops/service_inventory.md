...
|---|---|---|---|---|---|---|
| `apps/api-gateway` | Cloudflare Worker (API Gateway) | `apps/api-gateway/src/index.ts` | `/health`, `/ready`, `/v1/_debug/routes`, прокси по префиксам: `/v1/auth/*`, `/v1/users/*`, `/v1/content/*` (+ alias `/v1/api/content/*`), `/v1/media/*` (canonical media surface; fallback в `content-service`), `/v1/points/*`, `/v1/referral/*` | OpenAPI через gateway (единый контракт) | Нет собственной БД | `apps/api-gateway/src/index.ts`, `apps/api-gateway/wrangler.toml` |
