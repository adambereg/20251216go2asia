...
•	Background Sync: если пользователь совершил действие без сети (лайк, комментарий к посту/репосту в Space, чек-ин в квесте, реферальная регистрация), SW ставит задачу в очередь. При появлении соединения эти задачи автоматически отправляются на сервер. Пользователь при этом либо получает уведомление об успешной отправке, либо ничего не замечает (действие “применится” чуть позже). Это касается небольших операций, которые можно отложить.
...
•	Ответы на посты/репосты и комментарии в Space (социальная активность).
