...
Реализованы реакции и обсуждения в social-контуре Space (лайки, репосты, комментарии к постам/репостам), а также личные сообщения и групповые чаты – всё, что нужно для коммуникации внутри сообщества. Важно: inline-комментариев “под объектами” контентных модулей (Blog/Atlas/Pulse/RF/Rielt) нет — обсуждение этих объектов происходит через репост в Space и дальнейшее общение в контексте репоста. Есть разделы “Группы” (пользователь может вступать в интересующие сообщества, например “Бангкок Digital Nomads” или “Сёрферы Бали”), “События” (интеграция с Pulse – тут отображаются события, на которые пользователь подписан или участвует, с возможностью обсуждать их в чате) и “Люди рядом” (интеграция с Guru – на карте или списком можно увидеть поблизости PRO-участников, отметивших своё присутствие). 
...
## Репосты как единый механизм обсуждений контента (social-first)

В Go2Asia обсуждение контента не “встроено” в каждый модуль. Вместо этого любой объект экосистемы обсуждается через Space:

- Пользователь нажимает “Поделиться” в Blog/Atlas/Pulse/RF/Rielt → создаётся **репост** в Space.
- Репост — отдельный объект Space (пост типа `repost`) со ссылкой на исходный объект:
  - `author_id`
  - `target_type` (`blog_post` | `place` | `event` | `partner` | `quest` | `listing`)
  - `target_id`
  - `text` (optional) — контекст/мнение автора
  - `created_at`, `updated_at`
- Дальнейшее обсуждение и реакции происходят **в Space** (комментарии/реакции на пост/репост), а не под исходной сущностью.
...
Content Service – сервис контента сообщества и UGC. Он отвечает за хранение и управление пользовательскими публикациями (особенно в Space Asia) и другими материалами, создаваемыми пользователями. Хотя Atlas, Pulse, Blog имеют свои базы данных для структурированного контента, Content Service управляет менее формализованным контентом социального контура: посты, репосты и комментарии к постам/репостам (не inline-комментарии под исходными объектами), фотографии, возможно, личные заметки. Он обеспечивает создание, редактирование, удаление таких объектов, хранение медиафайлов (фотографий, видео) или интеграцию с CDN, а также модерационные атрибуты (флаги “пометить как спам”, статусы публикации). 
...
tasks – список заданий: каждое задание – отдельная структура (см. ниже).
...
Модель задания (Task): Каждое задание внутри квеста содержит: - type – тип задания: например, check-in (просто посетить место), photo (сделать фото), quiz (ответить на вопрос), visit (посетить и отметить, возможно через QR), comment (оставить отзыв/пост). - target_id – идентификатор целевого объекта задания, если применимо (например, id места из Atlas или события из Pulse, либо id поста из Space, если нужно создать пост). - reward_points – количество Points, начисляемых за выполнение этого задания. - require_proof – булево, требуется ли доказательство выполнения (например, загрузить фото, отсканировать QR на месте, подтвердить GPS).

Эти модели определяют, как Quest Service отдает данные фронтенду. Например, GET /v1/quests/{id} вернет JSON с полями, перечисленными выше, включая вложенный массив tasks. Клиентское приложение Quest Asia на основе этого строит интерфейс: карту маршрута (точки из locations), список заданий с описанием (из tasks), таймер или прогресс-бар (из meta.duration). Когда пользователь выполняет задание, клиент отправляет POST /v1/quests/{id}/checkin или другой соответствующий метод, содержащий task_id и подтверждение (фото или код) – Quest Service проверяет и помечает задание выполненным. В конечном счёте, при завершении всех заданий, Quest Service меняет статус квеста для пользователя и инициирует выдачу наград через Connect.
...
GET /v1/search?q=тайская кухня – **PLANNED / NOT IMPLEMENTED**: пример глобального поиска (целевая возможность). В текущем коде репозитория backend-эндпойнт `/v1/search` не обнаружен. См. `docs/decisions/search_strategy.md` и `docs/standards/search_conventions_v1.md`.
...
POST /v1/quests/create – создать новый квест (только для PRO). В теле – описание квеста, список точек/заданий. Новый квест, вероятно, сначала черновик.
POST /v1/quests/{id}/complete – отметить квест пройденным (вызывается, когда пользователь завершил все задания – возможно, не требуется вручную дергать, а система сама).
POST /v1/quests/{id}/checkin – прогресс по квесту: отметка выполнения одного задания. Тело запроса содержит данные выполнения (например, task_id, доказательство).
