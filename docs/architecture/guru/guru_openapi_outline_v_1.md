...
Текущий runtime baseline: живые upstream-sources в V1 = `rielt` + `rf` + `quest` + `atlas` + `pulse`; остальные declared domain adapters остаются explicit stubs и отражаются в `meta.sources_stub`.
