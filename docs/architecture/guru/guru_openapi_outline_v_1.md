...
Текущий runtime baseline: живые upstream-sources в V1 = `rielt` + `rf` + `quest`; остальные declared domain adapters остаются explicit stubs и отражаются в `meta.sources_stub`.
