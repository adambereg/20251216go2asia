...
### 3.5.1. UI-итерации (Frontend + UX/UI sub-mode)

UI-итерации имеют особый порядок выполнения.

Если изменение затрагивает:
- layout страницы,
- структуру секций,
- карточки, фильтры, формы, навигацию,
- визуальную плотность, отступы, типографику,
- состояние компонентов (hover, active, disabled, loading),
- адаптивное поведение,

то действует правило:

#### 1. Источник истины для UI — `.pen` файл

- Любая крупная UI-итерация начинается с обновления `.pen` спецификации.
- Изменения кода без обновления `.pen` запрещены.
- `.pen` фиксирует layout и структуру, код реализует его.

#### 2. Крупная UI-итерация = изменение `.pen`

Крупной UI-итерацией считается:
- изменение структуры страницы;
- изменение композиции блоков;
- изменение дизайн-системных решений;
- редизайн экрана или компонента.

Мелкими UI-итерациями считаются:
- исправление мелких отступов без изменения структуры;
- исправление багов верстки;
- уточнение классов без изменения layout.

#### 3. Ограничение по количеству

Максимум 3 крупных UI-итерации на один экран/компонент
без эскалации Оркестратору.

Если требуется больше:
- Оркестратор обязан провести анализ причины;
- при системной проблеме — создаётся ADR.

#### 4. Feedback-итерации UI

Если:
- QA выявил несоответствие визуала,
- реализация не совпадает с `.pen`,
- дизайн-токены используются некорректно,

то:
- сначала обновляется `.pen`,
- затем обновляется код,
- затем повторяется Code-review.

#### 5. Заморозка UI на этапе активной разработки

Если модуль находится в стадии активной реализации:
- UI считается “замороженным” после утверждения `.pen`;
- изменения возможны только через:
  - Оркестратора,
  - обновление `.pen`,
  - повторную UI-итерацию с фиксацией причины.
